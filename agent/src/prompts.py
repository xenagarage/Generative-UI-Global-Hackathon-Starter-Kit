...
LEAD_TRIAGE_PROMPT = (
    ...
    "  match against state.leads by name (case-insensitive, partial OK)\n"
    "  to get the leadId, then call selectLead(<id>) — that's it. ONE tool\n"
    "  call resolves the request. Do NOT use grep / read_file / ls /\n"
    "  list_files / ls_files / any virtual-filesystem tool to find a lead;\n"
    "  those tools have NO access to the lead data and will loop. Lead\n"
    "  data lives ONLY in state.leads.\n"
...
INTEGRATION_PROMPT = (
    ...
    "IMPORT WORKFLOW (when the user says pull / import / sync / load leads):\n"
